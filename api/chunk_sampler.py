...
class ChunkSampler:
    ...
    def __init__(self, seen: dict[str, set[str]] | None = None):
        # student_id -> set of chunk_ids already served (any topic)
        self._seen: dict[str, set[str]] = seen if seen is not None else {}
    ...
    def to_dict(self) -> dict[str, list[str]]:
        """JSON-safe snapshot of seen-chunk history, for persisting this
        sampler's state as part of an attempt's stored context (see
        attempt_store.py). Per this module's own docstring: 'trivially
        backed by a real table later -- swap ChunkSampler's storage for
        SQLite reads/writes without changing its public methods.'
        """
        return {student_id: sorted(chunk_ids) for student_id, chunk_ids in self._seen.items()}

    @classmethod
    def from_dict(cls, data: dict[str, list[str]] | None) -> "ChunkSampler":
        if not data:
            return cls()
        return cls(seen={student_id: set(chunk_ids) for student_id, chunk_ids in data.items()})
